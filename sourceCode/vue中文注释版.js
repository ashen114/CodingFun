...
(function(global, factory) {
  ...
  function mergeOptions(
    ...
    function mergeField(key) {
      ...
      var strat =
        strats[key] || defaultStrat; // //* 如果没有子节点就返回父节点，如果有子节点就返回子节点
      //获取子值还是父组的值
  ...
  function enter(vnode, toggleDisplay) {
    ...
    var isAppear =
      !context._isMounted || !vnode.isRootInsert; //是否已经调用过Mounted 生命周期函数 //   /*是否作为跟节点插入*/
  ...
  function parseHTML(
    ...
    function handleStartTag(match) {
      ...
      var unary =
        isUnaryTag$$1(tagName) || !!unarySlash; //函数匹配标签是否是 'area,base,br,col,embed,frame,hr,img,input,isindex,keygen, link,meta,param,source,track,wbr' //如果是/> 则为真
  ...
  function markStaticRoots(node, isInFor) {
    if (node.type === 1) {
      ...
      if (
        ...
        !(
          (
            node.children.length === 1 && node.children[0].type === 3 //如果只有一个子节点
          ) //属性节点
        )
      ) {
