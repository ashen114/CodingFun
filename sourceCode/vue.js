...
(function(global, factory) {
  ...
  /**
   * @param emptyObject Object.freeze() 方法可以冻结一个对象。一个被冻结的对象再也不能被修改。阻止修改现有属性的特性和值，并阻止添加新属性。
   */
  var emptyObject = Object.freeze({});

  // These helpers produce better VM code in JS engines due to their explicitness and function inlining.
  // 这些函数由于他们的明确性和内联性可以起到在JS引擎中生成更好的VM代码的作用
  /**
   * @function isUndef 判断数据是否为undefined或者null，返回true或者false
   * @param v 待判断的数据
   */
  function isUndef(v) {
  ...
  /**
   * @function isDef 判断数据是否不为undefined或者null，返回true或者false
   * @param v 待判断的数据
   */
  function isDef(v) {
    return v !== undefined && v !== null;
  ...
  /**
   * @function isTrue 判断数据是否为true，返回true或者false
   * @param v 待判断的数据
   */
  function isTrue(v) {
    return v === true;
  ...
  /**
   * @function isFalse 判断数据是否为false，返回true或者false
   * @param v 待判断的数据
   */
  function isFalse(v) {
    return v === false;
   ...
   */
  /**
   * @function isPrimitive 判断数据的类型是否为string（字符串）或者number（数字）或者Symbol（独一无二）或者boolean（布尔型），为其一则返回true或者false
   * @param value 待判断的数据
   */
   ...
   * 快速检查对象 - 这主要用于当我们知道数据为JSON基本类型时作为对象类型从原始类型中区分开来，
   */
  /**
   * @function isObject 判断数据是否不为null且类型为object（对象），返回true或者false
   * @param obj 待判断的对象
   */
   ...
   * 获取值的原始类型字符串，例如[object obejct]。
   */
  /**
   * @param _toString 简写Object.prototype.toString为_toString
   */
  // 例子：
  //  let a = ['ashen','apple']
  // _toString.call(a) // 输出"[object Array]" 其中Array为a的类型
  var _toString = Object.prototype.toString;
  /**
   * @function toRawType 如上例子所示，切割掉“[object ”及“]”，返回Array，Function，String，Object，RegExp等。
   * @param value 待切割的数据
   */
   ...
   * 严格的object对象类型检查，若为普通JavaScript对象类型则返回true
   */
  /**
   * 函数
   * 严格的object对象类型检查，若为普通JavaScript对象类型则返回true，返回true或者false
   */
  ...
  /**
   * 函数
   * 检查val是否为有效的数组索引值，返回true或者false
   */
  function isValidArrayIndex(val) {
    var n = parseFloat(String(val)); // 取出val有效数字部分
    // 判断val是否大于0且为整数且为有效数字
    return n >= 0 && Math.floor(n) === n && isFinite(val);
  }
  /**
   * 函数
   * 判断val是否为Promise对象类型，然后true或者false
   */
  function isPromise(val) {
    return (
      // isDef 判断数据是否不为undefined或者null，返回true或者false
      isDef(val) &&
      typeof val.then === "function" && // 判断是否含有then且为function类型
      typeof val.catch === "function" // 判断是否含有catch且为func类型
    );
  }
  ...
  /**
   * 方法
   * 将值转换为实际呈现的字符串，返回val的字符串类型
   */
  function toString(val) {
    return val == null
      ? "" // 若为null，则返回""
      : Array.isArray(val) || (isPlainObject(val) && val.toString === _toString) // 判断val是否为Array数组类型 || 或者是否为object类型
      ? JSON.stringify(val, null, 2) // 若val为数组或者object对象类型，则转为字符串类型（一般要将对象转字符串，用JSON.stringify）
      : String(val); // 将非数组或者非对象的数据转为字符串类型
  }

  /**
   * Convert an input value to a number for persistence.
   * If the conversion fails, return original string.
   * 将输入值转换为数字以进行持久化。如果转换失败，则返回原数值。
   */
  /**
   * 方法
   * 将输入值转换为数字以进行持久化。返回原数据val(若转换失败)或者转化为数字后的val(若转换成功)。
   */
  function toNumber(val) {
    var n = parseFloat(val); // 将val转换为数字
    return isNaN(n) ? val : n; // isNaN用于判断n是否为有效数值
  }

  /**
   * Make a map and return a function for checking if a key
   * is in that map.
   * 创建一个map对象且返回一个函数用于检查该map对象中是否存在键
   */
  /**
   * 方法
   * 将str切割为数组，循环插入map对象中，赋值都为true，若expectsLowerCase为true，则返回的键都为小写，否则返回str原大小写；
   * 返回map对象，其中的键可为小写（expectsLowerCase为true时）或者不变（expectsLowerCase不存在或者false时）
   * 最终，返回函数function(val){}
   */
  function makeMap(str, expectsLowerCase) {
    ...
    return expectsLowerCase
      ? function(val) {
          ...
          return map[val.toLowerCase()]; // 则将键转为小写再返回map，返回函数function(val)
        }
      : function(val) {
          // 若expectsLowerCase不存在或者为false
          return map[val]; // 则直接返回map，返回函数function(val)
        };
  }
  ...
  /**
   * 方法
   * 可以接收val参数，用于判断val是否为slot或component
   * 返回true或者false
   */
  ...
  /**
   * 方法
   * 可以接收val参数，用于判断val是否为key,ref,slot,slot-scope,is其中一个
   * 返回true或者false
   */
  ...
  function remove(arr, item) {
    if (arr.length) {
      //通过判断数组长度，检查数组是否有数据
      var index = arr.indexOf(item); // 获取待移除项位置
      if (index > -1) {
        // 判断待移除项的位置是否为-1，判断是否存在待移除项
        return arr.splice(index, 1); // 移除待移除项者这1项，返回新数组
      }
    }
  }
  ...
  /**
   * 变量
   * 检查对象是否具有该属性，通过call(obj,key)使用该方法
   * 例如 hasOwnProperty.call({name:'ashen'},'name')
   */
  ...
  var camelizeRE = /-(\w)/g;
  /**
   * 将str进行匹配，若存在“-单词”，则将去掉“-”并其“单词”替换成“首字母大写”返回
   * 例如str为v-for时，则返回vFor
   * （已通过cached缓存当前方法）
   */
  var camelize = cached(function(str) {
    return str.replace(camelizeRE, function(_, c) {
      // _为“-单词”，c为“单词”
      return c ? c.toUpperCase() : ""; // 将单词首字母大写
    });
  });
   ...
   * 将字符串大写
   */
  /** 将字符串的首字母大写（已通过cached缓存当前方法） */
  var capitalize = cached(function(str) {
    return str.charAt(0).toUpperCase() + str.slice(1); //str.charAt(index)可以根据 index 索引值找到当前字符串的值
  });
  ...
  /** 将除了边界的每个大写字母转为"-小写字母"，例如："OBJECT" 得到 "o-b-j-e-c-t"*/
   ...
   * *简单的bind polyfill适用于不支持它的环境，*例如PhantomJS 1。x。
   * 从技术上讲，我们不再需要它了*因为原生绑定在大多数浏览器中已经足够高了。
   * *但是删除它将意味着破坏能够在*PhantomJS 1.x中运行的代码，
   * 因此必须保留这些代码以实现向后兼容。
   */

  /* istanbul ignore next 忽略下一个*/
  /**
  ...
  function polyfillBind(fn, ctx) {
    function boundFn(a) {
      ...
      return l
        ? l > 1
          ? fn.apply(ctx, arguments) // 若参数长度大于1，让[arguments]作为fn的参数。并返回fn
          : fn.call(ctx, a) // 若参数长度不大于1，则让参数a作为fn的参数
